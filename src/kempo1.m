...
VERSION = 'KEMPO1 Version 3.3';

if nargin == 0  % LAUNCH GUI
  %-- Startup --
  try
    hsp = splash_screen(VERSION);
  ...
  prm = setParameters(filename,true);
  if isempty(prm)
    return;
  end
  handles = set_param(handles, prm);
...
%--------------------------------------------------------------------
function handles = set_param(handles,prm)
...
function varargout = btn_load_Callback(h, eventdata, handles, varargin)
  ...
  if filename
    filenameIn = [pathname,filename];
    prm = setParameters(filenameIn,true);
    set_param(handles, prm);
  end
...
%--------------------------------------------------------------------
function hfig = splash_screen(VERSION)
